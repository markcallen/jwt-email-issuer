...
import type * as IndexModule from "../src/index.js";
...
async function setupRouter(options = {}, overrides: MockOverrides = {}) {
  ...
  await mockModule("../src/index.js", () => ({ ...mocks }));
  const { createJwtRouter } = await import("../src/express.js");
...
describe("createJwtRouter", () => {
  ...
  it("responds with issuer metadata", async () => {
    ...
    const customIssuerMetadata = createMock<typeof IndexModule.issuerMetadata>(() => customMetadata);
  ...
  it("marks cookies as secure in production", async () => {
    ...
    try {
      const secureIssueToken = createMock<typeof IndexModule.issueToken>(async () => "secure-token");
  ...
  it("echoes the X-Email-Token header", async () => {
    const { app } = await setupRouter();
    const res = await request(app)
      .get("/api/echo-token")
      .set("X-Email-Token", "header-token");

    expect(res.status).toBe(200);
    expect(res.body).toEqual({
      message: "Server received X-Email-Token successfully!",
      receivedToken: "header-token"
    });
  });
});
